...
module dust_environment_sensor(fitfudge=0, version=1) {
    if ( version == 1) {
        difference() {
        ...
        difference() {
            translate([35,35,-1.5-fitfudge]) cylinder(r=35+fitfudge*2, h=3+1.5+fitfudge*2, $fn=128); // board + small components
            //mounting_holes();
        }
        translate([35,20,7+fitfudge/2]) cube([25+2*fitfudge,32+2*fitfudge,14+fitfudge],center=true); // xbee 
    }
    translate([3,41-2.54*2.5,5+fitfudge/2]) cube([2.4+2*fitfudge,14.75+2*fitfudge,10+fitfudge],center=true);
    translate([35,67,5+fitfudge/2]) cube([12.5+2*fitfudge,4.9+2*fitfudge,10+fitfudge],center=true);
    translate([22,61,0]) cylinder(r=10.8/2+fitfudge, h=16+fitfudge);
    
    translate([35,36,-31.7/2-fitfudge/2]) cube([59.1+2*fitfudge,30.27+2*fitfudge,31.7+fitfudge],center=true);
    translate([35,36,-5/2-fitfudge/2]) cube([59.1+fitfudge*2,5+30.27+2*fitfudge,5+fitfudge],center=true);
    translate([35+27,36,5-34-fitfudge]) cube([.6+2*fitfudge,2.83+2*fitfudge,10],center=true);
    translate([35-27,36,5-34-fitfudge]) cube([.6+2*fitfudge,2.83+2*fitfudge,10],center=true);
